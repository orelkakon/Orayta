...
import { deckPage } from '@/lib/feedShuffle';
...
export async function GET(req: NextRequest) {
  ...
  const seed = Number.isFinite(seedParam) ? seedParam >>> 0 : Math.floor(Math.random() * 4294967296);
  const page = Math.max(0, Math.floor(Number(req.nextUrl.searchParams.get('page')) || 0));

  const idsOf = { select: { id: true }, orderBy: { id: 'asc' as const } };
  const [cIds, rIdsAll, bIds, gIds, chIds, sIds, yahrzeitIds] = await Promise.all([
    N.citation ? prisma.citation.findMany(idsOf) : [],
    N.rabbi ? prisma.rabbi.findMany(idsOf) : [],
    N.book ? prisma.book.findMany(idsOf) : [],
    N.gematria ? prisma.gematria.findMany(idsOf) : [],
    N.chidush ? prisma.chidush.findMany(idsOf) : [],
    N.sikum ? prisma.sikumEntry.findMany(idsOf) : [],
    N.rabbi ? getYahrzeitRabbiIds() : new Set<string>(),
  ]);

  // Yahrzeit rabbis are shown once on page 0 only, so keep them out of the deck
  const rIds = rIdsAll.filter(r => !yahrzeitIds.has(r.id));
  const pick = (rows: { id: string }[], salt: FeedItemType) =>
    deckPage(rows.map(r => r.id), seed, salt, page, N[salt]);

  const [citations, rabbis, books, gematrias, chidushim, sikumEntries] = await Promise.all([
    N.citation ? prisma.citation.findMany({ where: { id: { in: pick(cIds, 'citation') } }, include: { locations: true } }) : [],
    N.rabbi ? prisma.rabbi.findMany({ where: { id: { in: pick(rIds, 'rabbi') } } }) : [],
    N.book ? prisma.book.findMany({ where: { id: { in: pick(bIds, 'book') } } }) : [],
    N.gematria ? prisma.gematria.findMany({ where: { id: { in: pick(gIds, 'gematria') } } }) : [],
    N.chidush ? prisma.chidush.findMany({ where: { id: { in: pick(chIds, 'chidush') } } }) : [],
    N.sikum ? prisma.sikumEntry.findMany({ where: { id: { in: pick(sIds, 'sikum') } }, include: { book: { select: { name: true, icon: true } } } }) : [],
  ]);

  // Yahrzeit rabbis: fetched separately, first page of the session only
  const yahrzeitIdArr = page === 0 ? Array.from(yahrzeitIds) : [];
  ...
  const regularItems = shuffle([
    ...
    ...rabbis.map(r => ({ type: 'rabbi' as const, id: r.id, reactions: getReactions('rabbi', r.id), data: { ...r } })),
