...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { name, author, icon, color } = (await req.json()) as { name: string; author?: string; icon?: string; color?: string };
  if (!name?.trim()) return NextResponse.json({ error: 'name required' }, { status: 400 });
  const book = await prisma.sikumBook.update({
    where: { id: params.id },
    data: { name: name.trim(), author: author?.trim() || null, icon: icon || '📒', color: color || null },
