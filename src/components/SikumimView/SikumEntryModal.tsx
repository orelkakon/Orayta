...
import { shareStory, sikumStory } from '@/lib/storyShare';
...
const ShareBtn = styled.a`
  ...
  &:hover { opacity: 0.88; }
`;
const ShareBtns = styled.div`display: flex; gap: ${theme.spacing.sm}; flex-wrap: wrap;`;
const StoryBtn = styled.button`
  display: inline-flex; align-items: center; gap: 6px;
  font-size: 0.82rem; font-weight: 600;
  padding: ${theme.spacing.xs} ${theme.spacing.md};
  background: linear-gradient(45deg, #f09433, #dc2743, #bc1888);
  color: white; border-radius: ${theme.radii.sm};
  ...
  return (
    <Modal onClose={onClose} title={modalTitle}>
      <Body>
        ...
        <ActionsRow>
          <ShareBtns>
            <ShareBtn href={waHref} target="_blank" rel="noopener noreferrer" onClick={() => trackShare()}>
            ...
            <StoryBtn onClick={() => shareStory(sikumStory(entry, bookName, bookAuthor))}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
              {HE.STORY_SHARE_IG}
            </StoryBtn>
          </ShareBtns>
          {(onEdit || onDelete) && (
