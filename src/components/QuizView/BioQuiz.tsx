...
export default function BioQuiz({ onAnswered }: Props) {
  ...
  const next = useCallback((list: Rabbi[]) => {
    ...
    const pool = list.filter(r => r.bio.length >= 40);
  ...
  useEffect(() => { if (all.length >= 4) next(all); }, [all, next]);
  ...
  const bio = question.bio.length > BIO_LEN
    ? question.bio.slice(0, BIO_LEN).trimEnd() + '...'
    : question.bio;
